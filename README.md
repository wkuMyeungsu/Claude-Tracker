...
```
┌─────────────────────────────────────┐
│ ● Claude Code Usage          [−][x] │  ← ●: 핀 버튼 (주황=고정, 회색=해제)
...
- 고정 ON: 창이 항상 위에 표시되고, idle 시 투명도 0.6 적용
- 고정 OFF: 투명도 효과 비활성화 (항상 불투명)
...
진행 바와 동일한 색상 임계값 / 동일한 dominant 로직 적용.
...
- **표시**: 상태 레이블에 `로컬 추정값` 표시. API 정상 시에는 표시 없음
...
activityStopped emit
  → UsagePopup::setIdle()
      → 2초 후 활성 라인 fade out (600ms)
      → 10초 후 팝업 투명도 0.6 (핀 고정 시에만)
```
