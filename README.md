...
```
┌─────────────────────────────────────┐
│ ●  Claude Code Usage  [● 토큰 발생] [−][x] │  ← 토큰 사용 중일 때 pill 표시
├─────────────────────────────────────┤
│  5시간 사용량                  62%  │
│  ████████████░░░░░░░                │
...
| 작업 종료 후 | 🟢→⚪ | 10초에 걸쳐 밝은 회색으로 페이드 |
| 대기 중 | ⚪ 밝은 회색 | 정적, 팝업 투명도 0.6 적용 |

- 토큰 사용 감지 시 LED가 초록 펄스로 즉시 전환
- 사용 종료 후 10초 페이드 완료 시점에 팝업도 투명화 (동기화)

### 활성 pill (타이틀바 우측)

토큰 사용 중일 때 `● 토큰 발생` 알림 pill이 타이틀바에 표시되고, 사용 종료 시 자동으로 사라집니다.

### 종료

타이틀바 `[x]` 버튼 클릭 → 앱 전용 스타일 확인 다이얼로그 → 확인 시 종료.
`[−]` 버튼은 팝업만 닫고 트레이에서 계속 실행.
...
300ms 조용 → activityStopped emit
  → UsagePopup::setIdle()
      → LED 10초 페이드 시작 (초록 → 회색)
      → [● 토큰 발생] pill 즉시 숨김
      → 10초 후 팝업 투명도 0.6 (LED 완료와 동기화)
```
...
| 항목 | 변경 전 | 변경 후 |
|------|---------|---------|
| 종료 버튼 | 팝업 하단 "종료" 버튼 | 타이틀바 `[x]` → 확인 다이얼로그 |
| 종료 다이얼로그 | 시스템 기본 QMessageBox | 앱 UI와 동일한 커스텀 스타일 |
| 활동 감지 디바운스 | 2000ms | 300ms |
| LED 페이드 시간 | 30초 | 10초 |
| LED idle 색상 | 어두운 회색 (#48484A) | 밝은 회색 (타이틀바에서 가시성 확보) |
| 투명화 트리거 | activity 종료 즉시 | LED 페이드 완료(10초) 시점에 동기화 |
| 초기 상태 | 불투명 (활동 없어도) | idle 상태 (앱 시작 시 바로 투명 적용) |
| 토큰 활동 표시 | 없음 | 타이틀바 `● 토큰 발생` pill |
